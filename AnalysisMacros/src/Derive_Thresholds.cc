...
void CorrectionObject::Derive_Thresholds(){
  ...
  double fitrange[n_trigger-1] = {50, 50, 50, 40, 10, 0 , 0, 0, 0};
  for(int i=0; i<n_trigger-1; i++){
    TString fitname = "fit";
    fitname +=  to_string(triggerVal[i+1]); 
    func[i] = new TF1(fitname,SmoothFit,triggerVal[i]-fitrange[i],
		      triggerVal[i+1]+100+fitrange[i],3);
    ...
    // cout << "test: single par 0,1,2 variation +/- " <<endl;
    // func[i]->SetParameter(0, func[i]->GetParameter(0)+func[i]->GetParError(0) );
    
    // thresholds_errUp[i] = fabs(func[i]->GetX(0.95*func[i]->GetParameter(2), triggerVal[i+1]-10, 800)-thresholds[i]);
    // thresholds09_errUp[i] = fabs(func[i]->GetX(0.9*func[i]->GetParameter(2), triggerVal[i+1]-10, 800)-thresholds09[i]);

    // cout <<func[i]->GetX(0.9*func[i]->GetParameter(2), triggerVal[i+1]-10, 800) <<endl;
    // func[i]->SetParameter(0, func[i]->GetParameter(0) - 2.*func[i]->GetParError(0) );

    // thresholds_errDown[i] = fabs(func[i]->GetX(0.95*func[i]->GetParameter(2), triggerVal[i+1]-10, 800)-thresholds[i]);
    // thresholds09_errDown[i] = fabs(func[i]->GetX(0.9*func[i]->GetParameter(2), triggerVal[i+1]-10, 800)-thresholds09[i]);

    // thresholds_err[i] = max(thresholds_errUp[i],thresholds_errDown[i]);
    // thresholds09_err[i] = max(thresholds09_errUp[i],thresholds09_errDown[i]);
    
    // cout <<func[i]->GetX(0.9*func[i]->GetParameter(2), triggerVal[i+1]-10, 800) <<endl;
    // func[i]->SetParameter(0, func[i]->GetParameter(0)+func[i]->GetParError(0) );
    
    // func[i]->SetParameter(0, func[i]->GetParameter(1)+func[i]->GetParError(1) );
    // cout <<func[i]->GetX(0.9*func[i]->GetParameter(2), triggerVal[i+1]-10, 800) <<endl;
    // func[i]->SetParameter(0, func[i]->GetParameter(1) - 2.*func[i]->GetParError(1) );
    // cout <<func[i]->GetX(0.9*func[i]->GetParameter(2), triggerVal[i+1]-10, 800) <<endl;
    // func[i]->SetParameter(0, func[i]->GetParameter(1)+func[i]->GetParError(1) );

    // func[i]->SetParameter(0, func[i]->GetParameter(2)+func[i]->GetParError(2) );
    // cout <<func[i]->GetX(0.9*func[i]->GetParameter(2), triggerVal[i+1]-10, 800) <<endl;
    // func[i]->SetParameter(0, func[i]->GetParameter(2) - 2.*func[i]->GetParError(2) );
    // cout <<func[i]->GetX(0.9*func[i]->GetParameter(2), triggerVal[i+1]-10, 800) <<endl;
    // func[i]->SetParameter(0, func[i]->GetParameter(2)+func[i]->GetParError(2) );
  ...
  gr09->Draw("ap");
  c->Print(CorrectionObject::_outpath+"plots/thresholds/"+"extrapolateLowestTrigger09.eps","eps");    
  ...
  mg->Draw("ap");  
  // gr095->Draw("ap");
  c2->Print(CorrectionObject::_outpath+"plots/thresholds/"+"extrapolateLowestTrigger.eps","eps");
  ...
  for(int i=0; i<n_trigger-1; i++){
      ...
      line09->Draw();     
      c1->Print(CorrectionObject::_outpath+"plots/thresholds/"+"HLT_PFJet"+to_string(triggerVal[i+1])+".eps","eps");
