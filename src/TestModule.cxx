...
namespace uhh2bacon {
  ...
  bool TestModule::process(Event & event) {
    ...
    if(is_mc){ /// apply for MC only
      // const TGenEventInfo & geninfo = event.get(h_genInfo);
      // TGenEventInfo* genInfo= new TGenEventInfo(geninfo);
    ...
    // float j3L1corr =1.; //TEST
    // float j1L1corr =1.; //TEST
    // float j2L1corr =0; //TEST
